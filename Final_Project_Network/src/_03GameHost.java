...
public class _03GameHost extends JFrame {
	...
	private JTextField t_input; // 필요 없음
	private JScrollPane scrollPane;
	private Timer timer;

	private List<Boolean> userReadyList = new ArrayList<>(); // 유저들의 준비 여부를 저장하는 리스트 추가
	private ArrayList<String> userAnswers = new ArrayList<>(); // 참가자들의 정답을 저장할 리스트
	...
	public void checkAnswer(String userAnswer) {
		if (userAnswer.equalsIgnoreCase(secretAnswer)) {
			...
			userAnswers.add(userAnswer); // 오답일 경우 리스트에 추가 (선택사항)
		}
	...
	private class ClientHandler extends Thread {
		...
		private void receiveMessages(Socket cs) {
			try {
				...
				printUserAnswerDisplay("User가 연결을 종료했습니다.");
			} catch (IOException e) {
