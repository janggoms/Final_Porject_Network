...
public class _03GameHost extends JFrame {
	...
	private boolean allUsersReady = false; // 추가: 모든 유저가 준비했는지 여부를 저장하는 변수
	...
	public _03GameHost(int port) {
		...
		this.port = port;
		startServer();
		s_button.setEnabled(false); // 초기에는 시작 버튼 비활성화
	}
	...
	public void setUserReady(int userNumber, boolean isReady) {
		if (userNumber >= 0 && userNumber < userReadyList.size()) {
			...
			allUsersReady = !userReadyList.contains(false); // 모든 유저가 준비했는지 여부 갱신
			if (allUsersReady && !timerStarted) { // 추가: 타이머가 시작되지 않았을 때만 시작 버튼 활성화
			...
			acceptThread = new Thread(() -> {
				while (acceptThread == Thread.currentThread()) {
					...
					try {
						...
						// sendToAllClients(USER_JOINED_MESSAGE); // 새로운 클라이언트 입장 메시지 전송
	...
	private class ClientHandler extends Thread {
		...
		private int userCount; // 각 클라이언트의 고유한 번호
		...
		private void receiveMessages(Socket clientSocket) {
			try {
				...
				while ((message = in.readLine()) != null) {
					printUserAnswerDisplay("User" + userCount + ": " + message);
					broadcasting(message);

					if (message.startsWith(USER_JOINED_MESSAGE)) {
						message.substring(USER_JOINED_MESSAGE.length());
						// printUserAnswerDisplay("User" + userCount + "가 입장했습니다.");
						// sendToAllClients(USER_JOINED_MESSAGE);
					} else {
						// 기존 클라이언트의 메시지인 경우
						printUserAnswerDisplay("User: " + message);
					}

					printUserAnswerDisplay("User" + userCount + "가 연결을 종료했습니다.");
					broadcasting("User" + userCount + "가 연결을 종료했습니다.");
				}
